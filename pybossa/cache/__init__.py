...
def get_key_to_hash(*args, **kwargs):
    ...
    key_to_hash = "".join(f":{i}" for i in args)
    # Attach any kwargs
    for key in sorted(kwargs.keys()):
        key_to_hash += f":{kwargs[key]}"
    return key_to_hash
...
def get_hash_key(prefix, key_to_hash):
    ...
    return f"{prefix}:{hashlib.md5(key_to_hash).hexdigest()}"
...
def cache(key_prefix, timeout=300):
    ...
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            key = f"{settings.REDIS_KEYPREFIX}::{key_prefix}"
...
def memoize(timeout=300):
    ...
    def decorator(f):
        @wraps(f)
        def wrapper(*args, **kwargs):
            key = f"{settings.REDIS_KEYPREFIX}:{f.__name__}_args:"
...
def delete_cached(key):
    ...
    """
    if util.redis_cache_is_enabled():
        key = f"{settings.REDIS_KEYPREFIX}::{key}"
...
def delete_memoized(function, *args, **kwargs):
    ...
    """
    if not util.redis_cache_is_enabled():
        return True
    key = f"{settings.REDIS_KEYPREFIX}:{function.__name__}_args:"
    if args or kwargs:
        key_to_hash = get_key_to_hash(*args, **kwargs)
        key = get_hash_key(key, key_to_hash)
        return bool(sentinel.master.delete(key))
    keys_to_delete = sentinel.slave.keys(pattern=f'{key}*')
    return (
        bool(sentinel.master.delete(*keys_to_delete))
        if keys_to_delete
        else False
    )
